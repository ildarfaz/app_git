...
export const Pagination: FC<IPagitation> = ({
  total = 0,
  activePage,
  onChangeParams,
}) => {
  const totalPage = Math.floor(total / 10);
  const pages = Array.from(Array(totalPage + 1).keys()).slice(
    activePage - 5 > 0 ? activePage - 5 : 1,
    activePage + 6,
  );
  ...
  return (
    <div className={styles.box}>
      <Button
        onClick={() => handlerPages(activePage - 1)}
        isDisabled={activePage === pages[0]}
      >
        {"Назад"}
      </Button>
      {pagesList}
